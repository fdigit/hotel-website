  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="bg-[#fbf8f1] py-16 sm:py-20 lg:py-28">
          <Reveal className="mx-auto grid max-w-[1380px] gap-14 px-5 md:px-10 lg:grid-cols-[.9fr_1.1fr] lg:gap-24 lg:px-14">
            <div>
              ...
              <h2 className="mt-5 font-headline-lg text-[36px] leading-[1.1] text-[#321017] sm:text-[42px] md:text-[58px]">A direct line to JAVIN.</h2>
              ...
              <div className="mt-10 space-y-4">
                {site.phones.map((phone) => (
                  <a key={phone.href} href={phone.href} className="group flex min-w-0 items-center justify-between gap-3 border-b border-black/10 py-5">
                    <span className="min-w-0">
                      <span className="block text-[10px] font-bold uppercase tracking-[0.18em] text-[#9d752e]">Telephone</span>
                      <span className="mt-1 block font-headline-md text-xl text-[#321017] min-[380px]:text-2xl">{phone.display}</span>
                    </span>
                    <span className="material-symbols-outlined text-[#5a0c1d] transition group-hover:translate-x-1">call</span>
                  </a>
                ))}
              </div>
              <a href={whatsappUrl("Hello JAVIN Hotels, I would like to make an enquiry.")} target="_blank" rel="noreferrer" className="mt-8 inline-flex w-full items-center gap-3 rounded-full bg-[#1f8f51] px-6 py-4 text-xs font-extrabold uppercase tracking-[0.12em] text-white min-[430px]:w-auto sm:px-8 sm:tracking-[0.14em]"><span className="material-symbols-outlined text-lg">chat</span>Chat on WhatsApp</a>
            </div>

            <div className="bg-[#efe8dc] p-6 sm:p-8 md:p-12">
              <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-[#9d752e]">Find us</p>
              <address className="mt-5 max-w-xl font-headline-md text-[26px] not-italic leading-[1.35] text-[#321017] sm:text-[30px] md:text-[38px]">{site.address}</address>
              <a href={mapUrl} target="_blank" rel="noreferrer" className="mt-8 inline-flex max-w-full items-center gap-2 border-b border-[#5a0c1d] pb-2 text-[11px] font-extrabold uppercase tracking-[0.1em] text-[#5a0c1d] sm:gap-3 sm:text-xs sm:tracking-[0.15em]">Open in Google Maps <span className="material-symbols-outlined shrink-0 text-base">north_east</span></a>
        ...
        <section className="bg-[#5a0c1d] py-16 text-white sm:py-20">
          <Reveal className="mx-auto max-w-4xl px-5 text-center">
            ...
            <h2 className="mt-4 font-headline-lg text-[36px] leading-tight sm:text-[42px] md:text-[56px]">Let us help make your arrival easy.</h2>
